...
$story = sprintf("select story_id, title, users.first_name, users.last_name, story_contents, date_posted, num_likes from stories join users on author_ID = users.pk_user_ID where story_ID = %d", $file_num);
...
$stmt->bind_result($story_ID,$story_title, $author_first, $author_last, $story_content, $date, $likes);
...
while($stmt->fetch()){
	...
	if(!isset($likes)){
        ...
        echo sprintf("Likes: %d", $likes);
    }
    
}

include 'script_likes.php';
echo ("<form method = 'POST'>");
echo ("<input type = 'submit' name = 'like' value = 'Like'>");
echo ("</form><br>");
if(isset($_POST['like'])){
    increase_likes($story_ID, $likes);
    header(sprintf("Location: logged_in_view.php?story_ID=%dstory_name=%s",$story_ID, rawurlencode(htmlspecialchars($story_title))));
}
