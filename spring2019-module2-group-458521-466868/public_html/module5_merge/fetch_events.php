...
//grab the requisite events from the database 
    ...
    if($user_check){
        ...
        if($user_check->fetch()){
            ...
            $stmt = $mysqli->prepare("SELECT pk_event_ID, title,event_date,start_time,end_time,event_location, tags FROM events JOIN users2events u2e ON u2e.event_ID = events.pk_event_ID WHERE u2e.user_ID=? AND event_date=?");
            if($stmt){
                //execute query

                $stmt ->bind_param('is', $pk_user_ID,$date);
                $stmt -> execute();
                $stmt -> bind_result($event_id, $fetched_title, $fetched_date, $fetched_start,$fetched_end, $fetched_location,$fetched_tags);
                //encode into json 
                $events = array();
                while($stmt -> fetch()){
                    $cur_event = array("event_id"=>$event_id, "title"=>$fetched_title, "date" => $fetched_date, "start" =>$fetched_start,"end"=>$fetched_end, "location" => $fetched_location, "tags"=>$fetched_tags);
